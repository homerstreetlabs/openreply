      ...
      <section className="border-b border-border bg-surface">
        <div className="mx-auto grid w-full max-w-7xl gap-10 px-5 py-14 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-20">
          <div>
            ...
            <p className="mt-8 text-sm font-bold uppercase tracking-wide text-muted">
            ...
            <ol className="mt-5 space-y-3">
              {template.playbook.map((step, index) => (
                <li key={step} className="grid gap-3 sm:grid-cols-[40px_1fr]">
                  <span className="flex h-8 w-8 items-center justify-center bg-accent text-sm font-black text-white">
                    {index + 1}
                  </span>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {relatedTemplates.map((item) => (
              <Link
                ...
                className="border border-border bg-surface p-5 transition hover:border-border-hover hover:bg-surface-hover"
              >
                <p className="text-xs font-semibold uppercase tracking-wide text-muted">
                  {item.category}
                </p>
