import { declaredStatement } from "../shared/ast.js";
...
const OBJECT_ANNOTATION = /\s*:\s*object\s*$/u;

/** Ban the broad object type on function inputs, including local aliases to object. */
export const noObjectParameters = {
	meta: {
		type: "problem",
		docs: {
			description:
	...
	create(context) {
		...
		const checkParameters = (node) => {
			...
			for (const parameter of node.params) {
				...
				context.report({
					...
					data: { parameter: parameterName(parameter, context.sourceCode, OBJECT_ANNOTATION) },
				});
			}
		};
		...
		return {
			...
			...forEachParameterOwner(checkParameters),
		};
	},
};
