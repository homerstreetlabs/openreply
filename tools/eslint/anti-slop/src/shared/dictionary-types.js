...
/**
 * Build the module-level view of type names a rule needs: aliases it can expand, interfaces
 * it can check for emptiness, and built-in names the module has shadowed (an imported or
 * locally declared `Record` is not TypeScript's `Record`).
 */
...
/** `{ readonly never?: never }`-style members carry no contract, so they read as empty. */
...
/** Bind an alias's type parameters to the arguments at a use site. */
...
/** Every value-position type a dictionary-shaped type exposes. */
...
function classifyAliasBroadTarget(type, environment, substitutions, resolvingAliases) {
	...
	if (unwrapped.type === "TSMappedType") {
		return isBroadMappedKey(mappedTypeConstraint(unwrapped), environment, substitutions)
			? { kind: "open dictionary" }
			: null;
	}
	if (unwrapped.type !== "TSTypeReference") return null;
...
/** Classify an explicit annotation as a target that would discard known value evidence. */
...
/** Expressions whose type the compiler already knows precisely from syntax alone. */
